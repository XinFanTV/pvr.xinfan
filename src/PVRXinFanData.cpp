...
#include "PVRXinFanData.h"

using namespace std;
using namespace ADDON;

PVRXinFanData::PVRXinFanData(void)
{
  m_iEpgStart = -1;
  m_strDefaultIcon =  "http://www.xinfan.tv/images/cc-logo1.jpg";
  m_strDefaultMovie = "";
...
int PVRXinFanData::GetChannelsAmount(void)
{
  return m_channels.size();
}

PVR_ERROR PVRXinFanData::GetChannels(ADDON_HANDLE handle, bool bRadio)
{
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVRXinFanChannel &channel = m_channels.at(iChannelPtr);
...
bool PVRXinFanData::GetChannel(const PVR_CHANNEL &channel, PVRXinFanChannel &myChannel)
{
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVRXinFanChannel &thisChannel = m_channels.at(iChannelPtr);
...
int PVRXinFanData::GetChannelGroupsAmount(void)
{
  return m_groups.size();
}

PVR_ERROR PVRXinFanData::GetChannelGroups(ADDON_HANDLE handle, bool bRadio)
{
  for (unsigned int iGroupPtr = 0; iGroupPtr < m_groups.size(); iGroupPtr++)
  {
    PVRXinFanChannelGroup &group = m_groups.at(iGroupPtr);
...
PVR_ERROR PVRXinFanData::GetChannelGroupMembers(ADDON_HANDLE handle, const PVR_CHANNEL_GROUP &group)
{
  for (unsigned int iGroupPtr = 0; iGroupPtr < m_groups.size(); iGroupPtr++)
  {
    PVRXinFanChannelGroup &myGroup = m_groups.at(iGroupPtr);
    if (!strcmp(myGroup.strGroupName.c_str(),group.strGroupName))
    {
      for (unsigned int iChannelPtr = 0; iChannelPtr < myGroup.members.size(); iChannelPtr++)
      {
        ...
        if (iId < 0 || iId > (int)m_channels.size() - 1)
          continue;
        PVRXinFanChannel &channel = m_channels.at(iId);
...
PVR_ERROR PVRXinFanData::GetEPGForChannel(ADDON_HANDLE handle, const PVR_CHANNEL &channel, time_t iStart, time_t iEnd)
{
  if (m_iEpgStart == -1)
    m_iEpgStart = iStart;
  ...
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVRXinFanChannel &myChannel = m_channels.at(iChannelPtr);
    ...
    while (iLastEndTime < iEnd && myChannel.epg.size() > 0)
    {
      ...
      {
        PVRXinFanEpgEntry &myTag = myChannel.epg.at(iEntryPtr);
...
int PVRXinFanData::GetRecordingsAmount(bool bDeleted)
{
  return bDeleted ? m_recordingsDeleted.size() : m_recordings.size();
}

PVR_ERROR PVRXinFanData::GetRecordings(ADDON_HANDLE handle, bool bDeleted)
{
  std::vector<PVRXinFanRecording> *recordings = bDeleted ? &m_recordingsDeleted : &m_recordings;

  for (std::vector<PVRXinFanRecording>::iterator it = recordings->begin() ; it != recordings->end() ; it++)
  {
    PVRXinFanRecording &recording = *it;
...
int PVRXinFanData::GetTimersAmount(void)
{
  return m_timers.size();
}

PVR_ERROR PVRXinFanData::GetTimers(ADDON_HANDLE handle)
{
  int i = 0;
  for (std::vector<PVRXinFanTimer>::iterator it = m_timers.begin() ; it != m_timers.end() ; it++)
  {
    PVRXinFanTimer &timer = *it;
